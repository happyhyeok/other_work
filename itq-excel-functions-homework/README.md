...
- 제출 완료 상태는 Google Sheets의 `진도관리` 시트를 기준으로 불러옵니다.
- 학생 이름을 선택하면 해당 학생의 제출현황과 제출횟수가 자동으로 함수 카드에 반영됩니다.
- **문제** 탭에서는 전체 문제와 각 함수의 제출횟수를 확인할 수 있습니다.
- **제출 완료** 탭에서는 현재 학생이 실제 제출한 함수만 확인할 수 있습니다.
- 제출 성공 후 팝업은 1.2초 뒤 자동으로 닫히며 제출현황을 다시 불러옵니다.
- 제출 상태의 최종 기준은 Google Sheets의 `제출기록`, `진도관리` 시트입니다.
